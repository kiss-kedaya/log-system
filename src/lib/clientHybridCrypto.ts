...
export async function encryptData(data: unknown): Promise<ArrayBuffer> {
  try {
    ...
    const headerSize = 1 + ENCRYPTED_AES_KEY_SIZE_LENGTH;
    ...
    const resultBuffer = new ArrayBuffer(
      headerSize + encryptedAesKey.length + ivBytes.length + contentBytes.length
    );
    const resultView = new DataView(resultBuffer);
    const resultUint8 = new Uint8Array(resultBuffer);

    // 写入协议版本
    resultView.setUint8(0, PROTOCOL_VERSION);

    // 写入加密AES密钥的长度
    resultView.setUint32(1, encryptedAesKey.length);

    // 写入加密的AES密钥
    resultUint8.set(encryptedAesKey, headerSize);

    // 写入IV
    resultUint8.set(ivBytes, headerSize + encryptedAesKey.length);
